...
// BGで予約済みの奇数コア（1,3,5,7,11,13+2*i）はスキップされます。
// ===============================================================
...
static inline bool IsReservedBgCore(int core) {
    ...
    if (core == SEQUENCER_GENERATOR_CORE)return true;
    if (core == SNAPSHOT_THREAD_CORE)    return true;
    for (int i = 0; i < NUM_RO_DISPATCHERS; ++i) {
        if (core == GET_RO_DISPATCHER_CORE(i)) return true;
    }
    return false;
}
