...
void *DeterministicScheduler::LockManagerThread(void *arg)
{
    ...
    while (true)
    {
        ...
        if (got_it == true)
        {
            ...
            // Only release locks if it's NOT a read-only transaction.
            // Read-only transactions never acquired locks.
        ...
        else
        {
            ...
            else if (executing_txns + pending_txns < MAX_ACTIVE_TXNS)
            {
                for (int i = 0; i < LOCK_BATCH_SIZE; i++)
                {
                    ...
                    {
                        // Read-only transaction: bypass the lock manager and push it
                        // directly to the ready queue.
                        scheduler->ready_txns_->push_back(txn);
                    }
                    else
                    {
                        // Normal transaction: request locks.
        ...
        while (!scheduler->ready_txns_->empty())
        {
            ...
            // If the transaction came from the lock manager, it was counted as
            // pending. If it's a read-only txn, it was never pending.
            // We can't distinguish here, so we only decrement pending_txns if it's > 0
            // A better approach would be to check if it's NOT read-only
            if (!(txn->has_read_only() && txn->read_only()))
            {
                pending_txns--;
            }
