...
#include <string>
#include <queue>
#include <pthread.h> // pthread_mutex_t for mutex_

#include "common/definitions.hh"
#include "common/utils.h" // AtomicQueueを使用するためにインクルード
...
public:
    virtual ~Client() {}
    virtual void GetTxn(TxnProtoExt **txn, int txn_id) = 0;
};
...
private:
    ...
    void FindParticipatingNodes(const TxnProtoExt &txn, set<int> *nodes);
    ...
    // Connection for receiving txn retries from scheduler.
    ...
    // read-onlyトランザクションをRunWriterからRunReaderに直接渡すためのキュー
    AtomicQueue<TxnProtoExt *> read_only_queue_;
};
