...
TEST(CustomerTest) {
  ...
  TxnProtoExt* secondary_keying = new TxnProtoExt();
...
TEST(NewTxnTest) {
  ...
  assert(txn_args->SerializeToString(SerializeToString(&txn_args_value)txn_args_value->data));

  // Initialize Transaction Generation
  TxnProtoExt* txn = tpcc->NewTxn(1, TPCC::INITIALIZE, txn_args_value, config);
  EXPECT_EQ(txn->txn_id(), 1);
  EXPECT_EQ(txn->txn_type(), TPCC::INITIALIZE);
  EXPECT_EQ(txn->isolation_level(), TxnProtoExt::SERIALIZABLE);
  EXPECT_EQ(txn->status(), TxnProtoExt::NEW);
...
TEST(MultipleTxnTest) {
  ...
  string args_string;
  args.SerializeToString(SerializeToString(&args_string)args_string->data);

  TxnProtoExt* txn = tpcc->NewTxn(0, TPCC::INITIALIZE, args_string, config);
  storage = new StorageManager(config, connection, simple_store, txn);
  tpcc->Execute(txn, storage);
  delete storage;
  delete txn;
