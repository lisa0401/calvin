...
DeterministicScheduler::DeterministicScheduler(Configuration* conf,
  ...
  for (int i = 0; i < NUM_WORKERS; i++) {
    ro_queues[i] = new AtomicQueue<TxnProto*>();
  }
...
void* DeterministicScheduler::LockManagerThread(void* arg) {
  ...
  int txns = 0; // <--- これがシステム全体のスループットカウンターになる
  double time = GetTime();
  int executing_txns = 0; // <--- これはRWロック待ちのTxnのみをカウント
  int pending_txns = 0;
  int batch_offset = 0;
  int batch_number = 0;
  // int test = 0;
  ...
  while (true) {
    ...
    if (got_it == true) {
      ...
      // 1. 完了したトランザクションをカウント (RO/RW両方)
      txns++;

      // 2. RWトランザクションの場合のみ、ロックを解放
      if (done_txn->read_only() == false) {
      ...
      if (batch_message == NULL) {
        ...
        for (int i = 0; i < LOCK_BATCH_SIZE; i++) {
          ...
          if (txn->read_only()) {
            // Read-Only トランザクション
            int worker_id = (scheduler->ro_counter_++) % NUM_WORKERS;
            scheduler->ro_queues[worker_id]->Push(txn);
          } else {
            // Read-Write トランザクション
    ...
    // Start executing any and all ready (RW) transactions to get them off our plate
    ...
    if (GetTime() > time + 1) {
      ...
      // 'txns' が正確な合計スループットを反映するようになった
      std::cout << "Completed " << (static_cast<double>(txns) / total_time)
                << " txns/sec, "
                //<< test<< " for drop speed , "
                << executing_txns << " executing (RW), " << pending_txns
                << " pending (RW), " << "\n"
                << task_output << "\n"
                << std::flush;
      // Reset txn count.
