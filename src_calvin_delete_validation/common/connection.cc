      ...
      deconstructor_invoked_(false)
{
    ...
    {
        if (it->second->node_id != config->this_node_id)
        { // Only remote nodes.
...
Connection *ConnectionMultiplexer::NewConnection(const string &channel)
{
    ...
    // Run()ループが処理を完了し、new_connection_が設定されるまで待機
...
bool Connection::GetMessage(MessageProto *message, int flags)
{
    zmq::message_t msg_;
    if (socket_in_->recv(&msg_, flags))
    {
        // Received a message.
...
// GetMessageBlocking関数も、変更後のGetMessageを呼び出すように修正
                                    ...
                                    double max_wait_time)
{
    ...
    do
    {
        // GetMessageにZMQ_NOBLOCKフラグを渡して呼び出す
        if (GetMessage(message, ZMQ_NOBLOCK))
        {
            // Received a message.
            return true;
        }
