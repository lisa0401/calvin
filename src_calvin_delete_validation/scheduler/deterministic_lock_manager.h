...
{
public:
    DeterministicLockManager(deque<TxnProtoExt*> *ready_txns, Configuration *config);
    virtual ~DeterministicLockManager() {}
    virtual int Lock(TxnProtoExt*txn);
    virtual void Release(const Key &key, TxnProtoExt*txn);
    virtual void Release(TxnProtoExt*txn);
...
private:
    ...
    struct LockRequest
    {
        LockRequest(LockMode m, TxnProtoExt*t) : txn(t), mode(m) {}
        TxnProtoExt*txn; // Pointer to txn requesting the lock.
    ...
    // txn.
    unordered_map<TxnProtoExt*, int> txn_waits_;
};
