...
#include <signal.h> // sigval_t のために必要

#include "common/utils.h"
#include "backend/storage.h"
#include "backend/simple_storage.h"

// これらのマクロは common/definitions.hh に移動することを推奨します。
// ここでは元のコードに合わせていますが、一元管理が望ましいです。
...
public:
    ...
    public:
        ...
        Latch()
        {
            ...
            pthread_cond_init(&cond_, NULL); // <-- ここも初期化を追加！
        }

        // デストラクタでミューテックスと条件変数を破棄することも重要です。
        // FetchingStorageのデストラクタでまとめて破棄する実装になっているため、
        // ここで個別に破棄する必要はないかもしれませんが、
        // Latchが独立してライフサイクルを持つ場合は必要になります。
        // ~Latch() {
