...
TEST(SimpleLockingTest) {
  deque<TxnProtoExt*> ready_txns;
  DeterministicLockManager lm(&ready_txns);
  vector<Key> key1, none;
  vector<TxnProtoExt*> owners;

  key1.push_back(Key("key1"));

  TxnProtoExt* t1 = (TxnProtoExt*) 1;
  TxnProtoExt* t2 = (TxnProtoExt*) 2;
  TxnProtoExt* t3 = (TxnProtoExt*) 3;
...
TEST(ThroughputTest) {
  ...
  for (int i = 0; i < 100000; i++) {
    //    txns.push_back(new TxnProtoExt());
    //    for (int j = 0; j < 10; j++)
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    while (ready_txns.size() > 0) {
      TxnProtoExt* txn = ready_txns.front();
